...
plot_prescriptions_quintile <- as.data.frame(prescriptions_by_quintile) %>% 
  ...
  geom_col()  +
  geom_errorbar( aes(x=`SIMD quintile`, ymin=`Mean number of prescriptions`- `Standard deviation`, ymax=`Mean number of prescriptions`+ `Standard deviation`), width=0.2, colour="black", alpha=0.8, size=0.3)
...
When we look at prescriptions per 1000 people, we see that there is quite a significantly greater number of prescriptions in the 41-45% overcrowding datazone group. However, this doesn't account for all datazones, only those that have records of permethrin and malathion prescriptions, so further investigations are needed. Further, there isn't a strong trend in the

Will also look at changes in scabies across Scotland over time (rise in scabies?)
...
combined_data_tidy_clean <- combined_data_tidy %>% 
  ...
  select(-hbt2014, -paid_date_month)
```
...
MAKE A LIL INTERACTIVE WIDGET FOR GOING OVER THE YEARS?? using shiny



```{r}
pres_datazones_spatial <- scottish_index_deprivation %>% 
  full_join(permeth_and_mala_summed_perpop, by="data_zone") %>% 
  filter(!st_is_empty(geometry))

map_data_with_prescriptions_plot <- pres_datazones_spatial %>%
  ggplot(aes(fill = datazone_pres_per_100)) +
  geom_sf(color=NA) +
  scale_fill_distiller(
    palette = "Blues", 
    direction = 1,  
    name = "No. of Prescriptions/100 people in datazone") +
      labs(title = "Number of prescriptions of scabies and malathion in datazones per 100 people", subtitle = "", fill = "No. of prescriptions") +
  theme_void() +
  theme(
...
map_data_with_prescriptions_plot

```

```{r}
pres_datazones_spatial_relevant <- pres_datazones_spatial %>% 
  select(data_zone, quintilev2, decilev2, house_o_crat, datazone_pres_per_100) %>% 
  filter(!is.na(datazone_pres_per_100)) 
```

```{r}

test_plot <- scottish_index_deprivation %>% 
  ggplot(aes(fill=inc_rankv2)) +
  geom_sf() +
  scale_fill_distiller(
    palette = "Blues", 
    direction = 1,  
...
We could look at this over health board as well, though p difficult lol
Could also look at just the major cities, i.e. edinburgh, glasgow, dundee etc and look at prescriptions at a more microscopic scale

Obviously, this doesn't really show very much haha. 
The SIMD data also contains quintiles, could look at correlation between quintiles and 
```{r}
#plot some kinda correlation between certain measures of inequality and the scabies prescription data
